    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 text-[clamp(14px,1.2vw,18px)]">
      ...
      <header className="bg-white shadow-sm border-b relative" id='header'>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            ...
            {/* Desktop Nav */}
            <nav className="hidden md:flex space-x-8 text-base lg:text-lg">
              <a
                href="#events"
      ...
      <section id="events" className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-4">Our Events</h2>
            <p className="text-base sm:text-lg lg:text-xl text-gray-600 max-w-3xl mx-auto">
              Discover amazing events, workshops, and competitions designed to enhance your skills and expand your network.
            </p>
          </motion.div>

          {/* Event lists */}
          {[
            { label: 'Upcoming Events', icon: Calendar, color: 'green', data: events.upcoming, status: 'upcoming' },
            { label: 'Ongoing Events', icon: Clock, color: 'orange', data: events.ongoing, status: 'ongoing' },
            { label: 'Past Events', icon: Users, color: 'purple', data: events.expired, status: 'expired' }
          ].map((section, idx) => {
            ...
            return (
              ...
              >
                <div className="flex items-center mb-8">
                  <Icon className={`w-6 sm:w-8 h-6 sm:h-8 text-${section.color}-600 mr-3`} />
                  <h3 className="text-2xl sm:text-3xl font-bold text-gray-900">{section.label}</h3>
                  <div className={`ml-4 bg-${section.color}-100 text-${section.color}-800 px-3 py-1 rounded-full text-sm sm:text-base font-medium`}>
                ...
                {section.data.length > 0 ? (
                  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {section.data.map((event) => (
                      <EventCard key={event.id} event={event} status={section.status} />
                    ))}
                  </div>
                ) : (
                  <div className="text-center py-12 bg-white rounded-lg border-2 border-dashed border-gray-200">
                    <Icon className="w-12 sm:w-16 h-12 sm:h-16 text-gray-400 mx-auto mb-4" />
                    <p className="text-gray-500 text-base sm:text-lg">No {section.label.toLowerCase()} at the moment</p>
                  </div>
                )}
              </motion.div>
            );
        ...
        <div className="grid lg:grid-cols-1 gap-16 mb-20">
          ...
          >
            <div className="flex items-center mb-6">
              ...
              <h3 className="text-2xl sm:text-3xl font-bold text-gray-900">InnovISE Club</h3>
            </div>
            <p className="text-sm sm:text-base md:text-lg text-gray-700 leading-relaxed mb-8">
              InnovISE is a premier technology and innovation club formed by the ISE (Information Science and Engineering) department of AMC Engineering College. We are dedicated to fostering creativity, learning, and collaboration among students and professionals. Our club organizes cutting-edge events, workshops, and competitions that bridge the gap between theoretical knowledge and practical application.
            </p>
            <div className="grid grid-cols-2 gap-4">
              {[
                { number: "50+", label: "Events Organized", color: "text-blue-600" },
                { number: "1000+", label: "Members", color: "text-purple-600" },
                { number: "25+", label: "Workshops", color: "text-green-600" },
                { number: "10+", label: "Competitions", color: "text-orange-600" },
              ].map((item, idx) => (
